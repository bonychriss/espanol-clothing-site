  ...
  const handleRemove = (idx) => {
    ...
    return (
      <div style={{ maxWidth: 600, margin: '2rem auto', background: '#fff', borderRadius: 16, boxShadow: '0 8px 32px rgba(0,0,0,0.08)', padding: '2rem', position: 'relative', zIndex: 10, overflow: 'visible' }}>
        <h2 style={{ color: '#FFD700', fontWeight: 800, fontSize: '1.5rem', marginBottom: 18, textAlign: 'center' }}>Your Cart</h2>
        {cart.length === 0 ? (
          <div style={{ textAlign: 'center', color: '#888', fontSize: '1.1rem', padding: '2rem 0' }}>Your cart is empty.</div>
        ) : (
          <>
            <ul style={{ listStyle: 'none', padding: 0, marginBottom: 24 }}>
              {cart.map((item, idx) => (
                <li key={idx} style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 18, borderBottom: '1px solid #eee', paddingBottom: 12 }}>
                  ...
                  <button onClick={() => handleRemove(idx)} style={{ background: '#FFD700', border: 'none', color: '#fff', fontWeight: 700, cursor: 'pointer', fontSize: 16, borderRadius: 6, padding: '0.4rem 0.8rem', marginLeft: 8 }}>Remove</button>
                </li>
              ))}
            </ul>
            <div style={{ fontWeight: 800, fontSize: '1.2rem', color: '#232F3E', textAlign: 'right', marginBottom: 24 }}>Total: {convertPrice(total)}</div>
            <button
